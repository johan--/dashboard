...
// Chart for Inventory by OPM
	$(function(){
		$http.get('/metricdashboard/get_inventory_opme').success(function(data) {
...
function inventory_opmUploadCtrl($scope, $http, $cookieStore, $location, $routeParams, $route, $resource) {
	...
	$scope.showContent = function($fileContent){
		...
		for (var i=0; i< $scope.jsonData.length; i++) {
			...
			$http.post('/metricdashboard/store_inventory_opme', store_inventory_opm_Data, config).success(function(data) {
			});
		};
	};
	$scope.complete = function(content) {
